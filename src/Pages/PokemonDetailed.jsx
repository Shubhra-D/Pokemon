...
const PokemonDetailed = () => {
  ...
  if (!pokemon) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height="100vh">
        <Typography variant="h6" color='#C75820'>No Pokémon found.</Typography>
      </Box>
    );
  }
  const renderEvolutionChain = (chain) => {
    ...
    return (
      <Box display="flex" gap={2} justifyContent="center" mt={2}>
        <Card sx={{ backgroundColor: '#EAC49D', textAlign: 'center', boxShadow: 3, borderRadius: 3, padding: 2 }}>
          <img src={chain.species.url ? `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${chain.species.url.split("/")[6]}.png` : ''} alt={chain.species.name} style={{ height: 100, marginBottom: 12 }} />
          <CardContent>
        ...
        {chain.evolves_to && renderEvolutionChain(chain.evolves_to[0])} {/* Recursively render next stage */}
      </Box>
    );
  };
  ...
  return (
    <Box p={3}>
      <Typography variant="h4" mb={3} textAlign="center" fontWeight="bold" color="#2a75bb">
        My name is {pokemon.name.charAt(0).toUpperCase() + pokemon.name.slice(1)} 
      </Typography>

      {/* Pokémon Card with Image */}
      <Card sx={{ backgroundColor: '#EAC49D', textAlign: 'center', boxShadow: 3, borderRadius: 3, padding: 2 }}>
        <img src={pokemon.image} alt={pokemon.name} style={{ height: 200, marginBottom: 12 }} />
        <CardContent>
          ...
          <Box mt={2}>
            ...
            {pokemon.abilities.map(ability => (
              <Typography key={ability} variant="body2">{ability}</Typography>
            ))}
          </Box>
          ...
          <Box mt={2}>
            ...
            {pokemon.moves.map(move => (
              <Typography key={move} variant="body2">{move}</Typography>
            ))}
          </Box>
        </CardContent>
      </Card>
      ...
      <Button onClick={()=>navigate('/pokemon')} sx={{color:'white' ,backgroundColor:'#EC407A'}} startIcon={<ArrowBackIosSharp/>} 
      >Go Back 
      </Button>
    </Box>
  );
